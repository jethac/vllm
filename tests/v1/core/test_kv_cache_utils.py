...
def test_unify_page_size_keeps_standard_attention_block_size_when_padding():
    ...
    # would create an unrepresentable standard-attention 5D view: the runner
    # splits the logical page into three 16-token kernel blocks, but the
    # padding gap exists only after every third kernel block. Upstream's
    # indexes_kv_by_block_stride opt-in padding keeps block_size unchanged.
    ...
    # The runner sets indexes_kv_by_block_stride from the backend
    # (FlashInfer's KV layout is num-blocks-first), which is what allows
    # padding this layer's pages without scaling its block size.
